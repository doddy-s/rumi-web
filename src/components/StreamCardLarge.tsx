...
export function StreamCardLarge({ stream, episode }: { stream: Stream, episode: Episode | null }) {
  return (
    <>
      <div className="h-[16rem] w-[12rem] overflow-hidden rounded-sm">
        ...
        <Link to="/watch/$streamId" params={{ streamId: stream?.consumetId }} search={{ episodeId: episode?.consumetId || null }} className="relative flex flex-col justify-between items-start h-full w-full p-2 
